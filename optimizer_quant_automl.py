"""
optimizer_quant_automl.py — Quant AutoML Optimizer (Bitvavo)
─────────────────────────────────────────────────────────────
Advanced AutoML version of the DCA Spot Backtest Dashboard.
Includes 5 extra AutoML features on top of the standard optimizer:
  1. Successive Halving / Early Pruning (Hyperband pruner)
  2. Automatic Search Space Narrowing (importance-guided)
  3. Meta-Learning Warm Start (cross-study seeding)
  4. Multi-Objective Pareto Optimization (Profit vs Drawdown)
  5. Filter Contribution Scoring (ON vs OFF ablation)
...
st.title("🤖 Quant AutoML Optimizer (Bitvavo)")
...
with st.sidebar.expander("🤖 Auto-Optimizer", expanded=False):
    ...
    opt_successive_halving = st.checkbox(
        "Successive Halving (Early Pruning)", value=False, key="opt_successive_halving",
        help="Fast trials on short data first. Worst 50% are eliminated early. Same quality as 5× more trials.",
    )
    opt_auto_narrow = st.checkbox(
        "Auto Search Space Narrowing", value=False, key="opt_auto_narrow",
        help="After each run, low-importance parameters are frozen and high-importance ranges are tightened around the best value.",
    )
    opt_narrow_top_pct = st.number_input(
        "Narrow range to ± %", value=20.0, min_value=5.0, max_value=50.0, step=5.0,
        key="opt_narrow_top_pct", disabled=not opt_auto_narrow,
        help="High-importance params are narrowed to ±X% of the best found value.",
    )
    opt_meta_warmstart = st.checkbox(
        "Meta-Learning Warm Start", value=False, key="opt_meta_warmstart",
        help="Seed the optimizer with the top params from all past studies. Gets smarter with every run.",
    )
    opt_pareto = st.checkbox(
        "Multi-Objective Pareto (Profit vs Drawdown)", value=False, key="opt_pareto",
        help="Optimize Profit AND minimize Drawdown simultaneously. Shows a Pareto frontier of trade-offs.",
    )
    opt_filter_contribution = st.checkbox(
        "Filter Contribution Scoring", value=False, key="opt_filter_contribution",
        help="Measures whether each entry filter actually helps. Runs backtest with filter ON vs OFF.",
    )
...
if run_opt_btn and date_from < date_to:
    ...
    def _load_meta_params() -> list[dict]:
        """Load top parameter sets from all past studies stored as JSON."""
        import json as _jmeta, pathlib as _plmeta
        _meta_file = _plmeta.Path(__file__).parent / "meta_params.json"
        if not _meta_file.exists():
            return []
        try:
            data = _jmeta.loads(_meta_file.read_text())
            return data.get("top_params", [])
        except Exception:
            return []

    def _save_meta_params(study_obj, top_n: int = 10):
        """Append best params from this study to the meta params file."""
        import json as _jmeta, pathlib as _plmeta
        _meta_file = _plmeta.Path(__file__).parent / "meta_params.json"
        existing = []
        if _meta_file.exists():
            try:
                existing = _jmeta.loads(_meta_file.read_text()).get("top_params", [])
            except Exception:
                existing = []
        # Add top N trials from current study
        _sorted_trials = sorted(
            [t for t in study_obj.trials if t.state == optuna.trial.TrialState.COMPLETE and t.value is not None],
            key=lambda t: t.value, reverse=True
        )[:top_n]
        for _t in _sorted_trials:
            _entry = {"params": _t.params, "value": _t.value, "study": _study_name}
            if _entry not in existing:
                existing.append(_entry)
        # Keep only most recent 100 entries
        existing = existing[-100:]
        _meta_file.write_text(_jmeta.dumps({"top_params": existing}, indent=2))

    # ── Filter contribution helper ────────────────────────────────────────────
    def _score_filter_contribution(p_base: dict, candles: list, days: float) -> dict:
        """For each active filter, compare score with filter ON vs OFF."""
        _results = {}
        _base_s = _score(p_base, candles, days)
        if _base_s is None:
            return {}
        _base_val = _base_s[opt_key]

        _filters = {
            "RSI Filter":         "rsi_filter",
            "SMA Filter":         "sma_filter",
            "EMA Filter":         "ema_filter",
            "Volume Filter":      "volume_filter",
            "BB Filter":          "bb_filter",
            "MACD Filter":        "macd_filter",
            "BOS Filter":         "bos_filter",
            "S&R Filter":         "sr_filter",
            "MA Cross Filter":    "ma_cross_filter",
            "ATR Dynamic":        "atr_dynamic",
            "Candle Pattern":     "candle_pattern_filter",
            "Regime Filter":      "regime_filter",
            "VP Filter":          "vp_filter",
        }
        for _fname, _fkey in _filters.items():
            if not p_base.get(_fkey, False):
                continue  # filter not active, skip
            _p_off = dict(p_base)
            _p_off[_fkey] = False
            _s_off = _score(_p_off, candles, days)
            if _s_off is None:
                continue
            _off_val = _s_off[opt_key]
            _delta   = _base_val - _off_val
            _pct     = (_delta / max(abs(_off_val), 1e-10)) * 100
            _results[_fname] = {
                "with_filter":    round(_base_val, 4),
                "without_filter": round(_off_val, 4),
                "delta":          round(_delta, 4),
                "delta_pct":      round(_pct, 1),
                "helpful":        _delta > 0,
            }
        return _results
    ...
    def objective(trial):
        ...
        # Report intermediate value for successive halving pruner
        trial.report(s[opt_key] if opt_key != "net" else s["_net_opt"], step=1)
    ...
    def objective_pareto(trial):
        """Two-objective: maximise net profit, minimise max drawdown."""
        # Reuse same param suggestions as objective()
        pair_t = trial.suggest_categorical("pair", [f"{p}|{iv}" for p, iv in valid_combos])
        sel_pair, sel_iv = pair_t.split("|", 1)
        store  = candle_store[(sel_pair, sel_iv)]
        _base_lo = max(float(opt_base_min), 0.1)
        ...
        _so_lo   = max(float(opt_so_size_min), 0.1)
        _so_hi   = max(float(opt_so_size_max), _so_lo + 0.1)
        base_o  = trial.suggest_float("base_order",   _base_lo, _base_hi) if opt_sweep_sizing else base_order_value
        so_size = trial.suggest_float("safety_order", _so_lo,   _so_hi)   if opt_sweep_sizing else safety_order_value
        tp      = trial.suggest_float("tp_pct",  max(opt_tp_min, 0.1),  max(opt_tp_max,  opt_tp_min  + 0.1)) if opt_sweep_tp   else take_profit_pct
        dev     = trial.suggest_float("dev_pct", max(opt_dev_min, 0.1), max(opt_dev_max, opt_dev_min + 0.1)) if opt_sweep_dev  else deviation_pct
        so      = trial.suggest_int(  "max_so",  int(opt_so_min), max(int(opt_so_max), int(opt_so_min)+1))    if opt_sweep_so   else max_safety_orders
        vs      = trial.suggest_float("vol_scale", max(opt_vs_min, 1.0), max(opt_vs_max, opt_vs_min + 0.01)) if opt_sweep_vs   else volume_scale
        step_m  = trial.suggest_float("step_mul",  max(opt_step_min, 0.1), max(opt_step_max, opt_step_min + 0.1)) if opt_sweep_step else step_multiplier
        rsi_thr = rsi_threshold
        p = _build_params(tp, dev, so, vs, step_m, rsi_thr, base_o, so_size, store["itvl_min"])
        s = _score(p, store["train"], store["train_days"])
        if s is None:
    ...
    if opt_successive_halving:
        _sampler = optuna.samplers.TPESampler(seed=42)
        _pruner  = optuna.pruners.HyperbandPruner(
            min_resource=10, max_resource=int(opt_trials), reduction_factor=3
        )
    else:
        _sampler = optuna.samplers.TPESampler(seed=42)
    ...
    if opt_meta_warmstart:
        _meta_entries = _load_meta_params()
        _added = 0
        for _me in _meta_entries:
            try:
                # Only enqueue if the params are compatible with current search space
                _mp = _me["params"]
                if "pair" in _mp and _mp["pair"].split("|")[0] in (opt_pairs or [trading_pair]):
                    study.enqueue_trial(_mp)
                    _added += 1
            except Exception:
                pass
        if _added:
            st.info(f"🧠 Meta-learning: seeded study with **{_added}** top params from past runs.")

    # ── Pareto study setup ────────────────────────────────────────────────────
    if opt_pareto:
        _pareto_study = optuna.create_study(
            study_name=_study_name + "_pareto",
            directions=["maximize", "minimize"],  # profit max, drawdown min
    ...
    try:
        ...
        if "dynamic value space" in str(_ve) or "CategoricalDistribution" in str(_ve):
            # Stale DB has different pair/timeframe choices — delete and restart fresh
            import os as _os_ve
    ...
    # Save this study's best params for future meta-learning
    if opt_meta_warmstart:
        try:
            _save_meta_params(study, top_n=10)
            st.caption("🧠 Top params saved to `meta_params.json` for future warm starts.")
        except Exception:
            pass
    ...
    # ── Filter Contribution Scoring ───────────────────────────────────────────
    if opt_filter_contribution:
        st.subheader("🔍 Filter Contribution Scoring")
        st.caption(
            f"Each filter tested: score **with** vs **without** filter on training data. "
            f"Metric: **{opt_metric}**."
        )
        _all_candles_fc = best_store["train"]
        with st.spinner("Scoring filter contributions…"):
            _fc_results = _score_filter_contribution(best_p, _all_candles_fc, best_store["train_days"])

        if _fc_results:
            _fc_rows = []
            for _fn, _fv in sorted(_fc_results.items(), key=lambda x: x[1]["delta_pct"], reverse=True):
                _fc_rows.append({
                    "Filter":          _fn,
                    "With Filter":     _fv["with_filter"],
                    "Without Filter":  _fv["without_filter"],
                    "Delta":           _fv["delta"],
                    "Delta %":         _fv["delta_pct"],
                    "Verdict":         "✅ Helps" if _fv["helpful"] else "❌ Hurts",
                })
            _fc_df = pd.DataFrame(_fc_rows)
            st.dataframe(_fc_df, use_container_width=True)

            _hurting = [r["Filter"] for r in _fc_rows if not _fc_results[r["Filter"]]["helpful"]]
            if _hurting:
                st.warning(
                    f"⚠️ These filters reduce performance — consider disabling them: "
                    + ", ".join(f"**{f}**" for f in _hurting)
                )
            else:
                st.success("✅ All active filters contribute positively.")
        else:
            st.info("No active filters to score, or not enough trades.")

    # ── Auto Search Space Narrowing ───────────────────────────────────────────
    if opt_auto_narrow:
        try:
            _importance = optuna.importance.get_param_importances(study)
            _total_imp  = sum(_importance.values()) or 1.0
            _bp_vals    = study.best_params
            _narrow_suggestions = []
            _freeze_suggestions = []
            for _pname, _pimp in _importance.items():
                _rel = _pimp / _total_imp
                if _rel < 0.02:
                    _freeze_suggestions.append(_pname)
                elif _rel > 0.10 and _pname in _bp_vals:
                    _bval = _bp_vals[_pname]
                    if isinstance(_bval, float):
                        _margin = abs(_bval) * (opt_narrow_top_pct / 100)
                        _narrow_suggestions.append(
                            f"**{_pname}**: narrow to [{_bval - _margin:.3f}, {_bval + _margin:.3f}]"
                        )
                    elif isinstance(_bval, int):
                        _margin = max(1, int(abs(_bval) * (opt_narrow_top_pct / 100)))
                        _narrow_suggestions.append(
                            f"**{_pname}**: narrow to [{_bval - _margin}, {_bval + _margin}]"
                        )

            st.subheader("🔬 Auto Search Space Narrowing")
            if _narrow_suggestions:
                st.markdown("**Tighten these ranges for the next run (high importance):**")
                for _s in _narrow_suggestions:
                    st.markdown(f"  • {_s}")
            if _freeze_suggestions:
                st.markdown("**Consider freezing these (importance < 2%):**")
                st.markdown("  • " + ", ".join(f"`{p}`" for p in _freeze_suggestions))
            if not _narrow_suggestions and not _freeze_suggestions:
                st.info("Need more trials to compute reliable importance scores.")

            # Save narrowing recommendations to a JSON file next to the script
            import json as _json_narrow, pathlib as _pl_narrow, datetime as _dt_narrow
            _narrow_path = _pl_narrow.Path(__file__).parent / f"{_study_name}_narrowing.json"
            _narrow_data = {
                "study": _study_name,
                "timestamp": _dt_narrow.datetime.now().isoformat(),
                "best_params": _bp_vals,
                "narrow": _narrow_suggestions,
                "freeze": _freeze_suggestions,
            }
            _narrow_path.write_text(_json_narrow.dumps(_narrow_data, indent=2))
            st.caption(f"💾 Narrowing recommendations saved to `{_narrow_path.name}`")
        except Exception as _e:
            st.info(f"Auto narrowing needs ≥ 20 trials. ({_e})")

    # ── Pareto optimization run ───────────────────────────────────────────────
    if opt_pareto:
        st.subheader("🎯 Multi-Objective Pareto Frontier")
        st.caption("Each dot = one trial. X = Net Profit, Y = Max Drawdown. Best params are bottom-right.")
        with st.spinner(f"Running {int(opt_trials)} Pareto trials (Profit vs Drawdown)…"):
            _pareto_study.optimize(objective_pareto, n_trials=int(opt_trials), show_progress_bar=False)

        _pareto_trials = [t for t in _pareto_study.trials
                          if t.state == optuna.trial.TrialState.COMPLETE and t.values is not None]
        if _pareto_trials:
            import plotly.graph_objects as _pgo_p
            _px = [t.values[0] for t in _pareto_trials]  # net profit
            _py = [t.values[1] for t in _pareto_trials]  # drawdown
            # Pareto front: non-dominated solutions
            _pareto_front = []
            for _ti, _t in enumerate(_pareto_trials):
                _dominated = False
                for _tj, _t2 in enumerate(_pareto_trials):
                    if _ti != _tj and _t2.values[0] >= _t.values[0] and _t2.values[1] <= _t.values[1]:
                        _dominated = True
                        break
                if not _dominated:
                    _pareto_front.append(_t)
            _pfx = [t.values[0] for t in _pareto_front]
            _pfy = [t.values[1] for t in _pareto_front]

            _pfig = _pgo_p.Figure()
            _pfig.add_trace(_pgo_p.Scatter(
                x=_px, y=_py, mode="markers",
                marker=dict(color="steelblue", size=6, opacity=0.5),
                name="All trials",
            ))
            _pfig.add_trace(_pgo_p.Scatter(
                x=_pfx, y=_pfy, mode="markers+lines",
                marker=dict(color="gold", size=10, symbol="star"),
                line=dict(color="gold", width=2),
                name="Pareto front",
            ))
            _pfig.update_layout(
                title="Pareto Frontier: Net Profit vs Max Drawdown",
                xaxis_title="Net Profit (EUR)", yaxis_title="Max Drawdown (%)",
            ...
            _pf_rows = []
            for _pft in sorted(_pareto_front, key=lambda t: t.values[0], reverse=True):
                _row = {"Net Profit (EUR)": round(_pft.values[0], 2),
                        "Max DD (%)": round(_pft.values[1], 2)}
                _row.update({k: round(v, 3) if isinstance(v, float) else v
                              for k, v in _pft.params.items() if k != "pair"})
                _pf_rows.append(_row)
            st.dataframe(pd.DataFrame(_pf_rows), use_container_width=True)
            st.caption("💡 Pick a row: low drawdown = conservative, high profit = aggressive.")
