    ...
    for _t in study.trials:
        if (_t.state == optuna.trial.TrialState.COMPLETE and
            ...
            trial_rows.append({
                ...
                "BOS Recency":      _pr.get("bos_rec", "—") if bos_filter else "—",
                "SR Lookback":      _pr.get("sr_lb", "—") if sr_filter else "—",
                "SR Proximity %":   round(_pr.get("sr_prox", 0), 2) if sr_filter else "—",
                "SL %":             round(_pr.get("sl_pct", 0), 2) if stop_loss_enabled else "—",
                "MA Fast":          _pr.get("ma_fast", "—") if ma_cross_filter else "—",
                "MA Slow":          _pr.get("ma_slow", "—") if ma_cross_filter else "—",
                "ATR TP Mult":      round(_pr.get("atr_tp", 0), 2) if atr_dynamic else "—",
                "ATR SL Mult":      round(_pr.get("atr_sl", 0), 2) if atr_dynamic else "—",
                "ADX Threshold":    round(_pr.get("adx_thr", 0), 1) if (regime_filter and regime_mode != "Any") else "—",
                "Trades":           "–",
                "Win Rate (%)":     "–",
                "Net Profit (EUR)": "–",
                "ROI (%)":          "–",
